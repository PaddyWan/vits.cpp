...
public:
    ...
    struct ggml_cgraph* build_graph_part_two(struct ggml_context* ctx, ggml_tallocr* allocr, struct ggml_tensor* input_ids, struct ggml_tensor * cum_duration, struct ggml_tensor* prior_means, struct ggml_tensor* prior_log_variances, struct ggml_tensor* speaker_embeddings, int output_length);
    ...
    struct ggml_tensor* wavenet_graph(struct ggml_context* ctx, ggml_tallocr*  allocr, struct ggml_tensor* input, struct ggml_tensor* speaker_embedding);
    struct ggml_tensor* flow_graph(struct ggml_context* ctx, ggml_tallocr*  allocr, struct ggml_tensor* inputs, struct ggml_tensor* conditioning, bool reverse);
    std::pair<struct ggml_tensor*, struct ggml_tensor*> flow_graph_layer(struct ggml_context* ctx, ggml_tallocr*  allocr, struct ggml_tensor* inputs, struct ggml_tensor* conditioning, bool reverse);
    struct ggml_tensor* hifigan_graph(struct ggml_context* ctx, ggml_tallocr*  allocr, struct ggml_tensor * input_ids, struct ggml_tensor* global_conditioning);
    ...
    size_t sample_rate() const;//returns the sample rate of the audio generated by this model, if not specified by the model, we will fall back to the default 16000Hz
};
...
VITS_API void vits_init_backend(int thread);
VITS_API void vits_free_backend();
